...
public class DarkLightTreeStructure extends ElementalmasterModElements.ModElement {
	public DarkLightTreeStructure(ElementalmasterModElements instance) {
		super(instance, 695);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 100000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("elementalmaster", "darklighttree"));
						if (template == null)
							return false;
